...
class TrackVisualizer(Visualizer):
    def __init__(self, img_rgb, metadata=None, scale=1.0, instance_mode=ColorMode.IMAGE, 
                 last_frame=None, last_black_mask=None):
        super().__init__(
            img_rgb, metadata=metadata, scale=scale, instance_mode=instance_mode
        )
        self.cpu_device = torch.device("cpu")
        self.black_mask = self.gen_black_mask(*img_rgb.shape[:2])
        self.last_black_mask = last_black_mask
        self.last_frame = last_frame
    
    def gen_black_mask(self, img_height, img_width):
        black_mask = np.zeros((img_height, img_width), dtype=np.bool_)
        top_black = (0, 0, img_height // 9, img_width)
        bottom_black = (8 * img_height // 9, 0, img_height // 9, img_width)
        left_black = (
            img_height // 9,
            5 * img_width // 16 - 56 * img_width // 1920,
            7 * img_height // 9,
            56 * img_width // 1920
        )
        right_black = (
        ...
        self.black_boxes = [top_black, bottom_black, left_black, right_black]
        for ph, pw, h, w in self.black_boxes:
            black_mask[ph:ph+h, pw:pw+w] = 1
        return black_mask
    ...
    def draw_autostereoscopy(self, predictions):
        """
        Draw instance-level prediction results on an image.
        Args:
            predictions (Instances): the output of an instance detection/segmentation
        ...
        if classes is None:
            return self.output, cv2.cvtColor(self.output.get_image(), cv2.COLOR_RGB2GRAY), self.black_mask
        black_mask = self.black_mask.copy()
        mask_total = np.zeros((self.output.height, self.output.width), dtype=np.bool_)
        for x, score in zip(masks, scores):
            # 计算识别物体的长度和宽度
            y_coords, x_coords = np.nonzero(x)
            ...
            if score >= 0.5:
                mask_total |= x
        black_mask[mask_total == True] = 0
        # 与上一帧的相似度、mask与上一帧的mask的相似度
        # 如果两帧相似，但是mask差别过大，则取mask求和后最小的一个
        gray_img = cv2.cvtColor(self.output.get_image(), cv2.COLOR_RGB2GRAY)
        if self.last_frame is not None and self.last_black_mask is not None:
            diff_frame = np.mean(cv2.absdiff(self.last_frame, gray_img))
            diff_mask = np.mean(cv2.absdiff(self.last_black_mask.astype(np.uint8), black_mask.astype(np.uint8)))
            # with open("diff.log", "a+", encoding="utf-8") as f:
            #     f.write(f"{diff_frame}, {diff_mask}\n")
            replace_mask = False
            if diff_frame < 15 and diff_mask > 0.05:
                last_sum = np.sum(self.last_black_mask)
                curr_sum = np.sum(black_mask)
                if last_sum < curr_sum:
                    black_mask = self.last_black_mask
                    replace_mask = True
            # 去除孤立的较小的mask区域
            if not replace_mask:
                num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(black_mask.astype(np.uint8), connectivity=4, ltype=None)
                for i in range(1, num_labels):
                    black_mask[labels == i] = 0 if stats[i][4] < self.black_mask.size // 580 else 1
        self.draw_binary_mask(black_mask, (0,0,0), alpha=1)
        return self.output, gray_img, black_mask
